...
class HaystackDocumentStore:
    def __init__(self, store_type: str = "ElasticsearchDocumentStore", **kwargs):
        if store_type not in ["ElasticsearchDocumentStore"]:
            raise Exception(
                "At the moment we only deal with one of these types: ElasticsearchDocumentStore"
            )
    ...
    for idx_article, article in enumerate(tqdm(squad_data, unit="article")):
        ...
        if question_id != last_question_id:
            if question_id != 0:
                ...
                dict_DPR["hard_negative_ctxs"] = hard_negative_ctxs
                if not hard_negative_ctxs:
                    logging.error(
                        f"No retrieved candidates for article {article_title}, with question {dict_DPR['question']}"
                    )
            ...
            dict_DPR = {
                "question": article["paragraphs"][0]["qas"][0]["question"],
                "answers": [answer["text"] for answer in article["paragraphs"][0]["qas"][0]["answers"]],
                "positive_ctxs": [
                    {"title": article_title, "text": article["paragraphs"][0]["context"], "passage_id": ""}],
                "negative_ctxs": [],
                "hard_negative_ctxs": [],
            }
        else:
            dict_DPR["answers"].append(article["paragraphs"][0]["qas"][0]["answers"][0]["text"])
            dict_DPR["positive_ctxs"].append(
                {"title": article_title, "text": article["paragraphs"][0]["context"], "passage_id": ""})

        last_question_id = question_id

        if idx_article == len(squad_data) - 1:
            hard_negative_ctxs = get_hard_negative_contexts(
                retriever=retriever, question=dict_DPR["question"], answers=dict_DPR["answers"],
...
def main(
    ...
    document_store: Union[ElasticsearchDocumentStore] = store_factory.get_document_store()
